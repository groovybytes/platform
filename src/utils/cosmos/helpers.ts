...
export async function createRoleDefinition(role: RoleDefinition): Promise<RoleDefinition> {
  ...
  export async function assignRolesToUser(userId: string, resourceType: string, resourceId: string, roles: string[], assignedBy: string, isGuest: boolean = false): Promise<AssignedRole> {
    const assignedRole: AssignedRole = {
      ...
      resourceId,
      resourceType,
      assignment_type: isGuest ? "guest" : "member",
