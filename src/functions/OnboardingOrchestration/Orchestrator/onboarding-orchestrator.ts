...
import type { Membership, OnboardingStatus } from '~/types/operational';
...
const OnboardingOrchestratorHandler: OrchestrationHandler = function* (context: OrchestrationContext) {
  ...
  try {
    ...
    yield context.df.callActivity(SaveOnboardingStatus.Name, status);
...
function* handleInviteOnboarding(
  ...
  yield context.df.callActivity('SendWelcomeResources', {
    userId,
    resourceType,
    resourceId
  });
...
function* handleWorkspaceOnboarding(
  ...
  yield context.df.callActivity('SetupInitialWorkspaceContent', {
    userId,
    workspaceId
  });
...
function* handleProjectOnboarding(
  ...
  yield context.df.callActivity('SetupInitialProjectContent', {
    userId,
    projectId,
    workspaceId
  });
  ...
  yield context.df.callActivity('SendProjectWelcomeResources', {
    userId,
    projectId,
    workspaceId
  });
...
export interface OnboardingInput {
  ...
  membershipId?: string;     // For membership id
